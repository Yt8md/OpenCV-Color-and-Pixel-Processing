...
image = cv2.imread("Eagle_in_Flight.jpg",0)
```
...
```python
image_crop = image_color[20:450,180:550]
plt.imshow(image_crop)
```

#### 8. Resize the image up by a factor of 2x.
```python
res= cv2.resize(image_crop,(200*2, 200*2))
```
...
```python
apollo_image = cv2.imread('Apollo-11-launch.jpg',cv2.IMREAD_COLOR)
img_rgb = cv2.cvtColor(apollo_image,cv2.COLOR_BGR2RGB)
img_rgb.shape
```
